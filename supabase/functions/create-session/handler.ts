...
export interface CreateSessionDeps {
  ...
  generateTestNumber?: () => string;
  getPublicUrl?: () => string | undefined;
  now?: () => Date;
}

export async function handleCreateSession(req: Request, deps: CreateSessionDeps) {
  ...
  try {
    patientAgeYears = calculateAgeYears(patient.date_of_birth, deps.now?.() ?? new Date());
  ...
  try {
    accessCode = await generateUniqueTestNumber(supabase, deps.generateTestNumber ?? generateTestNumber);
  } catch (error) {
  ...
  const baseUrl = sessionBaseUrl(req, deps.getPublicUrl?.());
...
function sessionBaseUrl(req: Request, configuredPublicUrl?: string): string {
...
function calculateAgeYears(dateOfBirth: string, todayDate: Date): number {
...
async function generateUniqueTestNumber(supabase: SupabaseClient, generateCode: () => string): Promise<string> {
