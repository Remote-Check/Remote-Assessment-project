...
export function NamingTask() {
  ...
  const savedData = state.tasks.naming || { currentIndex: 0, answers: {} };
  
  const [currentIndex, setCurrentIndex] = useState(savedData.currentIndex);
  const [answers, setAnswers] = useState<Record<string, string>>(savedData.answers);

  // Sync back to global store when local state changes
  useEffect(() => {
    updateTaskData('naming', { currentIndex, answers });
  }, [currentIndex, answers, updateTaskData]);
