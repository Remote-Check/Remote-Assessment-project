...
export function SessionValidation() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      // For now, length check is our validation
      if (token.length < 3) {
        setError("קוד המבחן אינו תקין. הקוד שהזנת אינו קיים במערכת.");
        return;
      }
      
      // Valid! Initialize session and go to welcome
      startNewAssessment();
      navigate("/patient/welcome");
    }, 1500);

    return () => clearTimeout(timer);
  }, [token, navigate, startNewAssessment]);
