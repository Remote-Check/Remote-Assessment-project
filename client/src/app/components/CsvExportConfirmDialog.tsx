      ...
      >
        ...
        <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
          ...
          <button
            ...
            aria-busy={exporting}
            className="flex min-h-12 items-center justify-center gap-2 rounded-xl bg-black px-5 py-2 font-bold text-white hover:bg-gray-800 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {exporting && <Loader2 className="h-5 w-5 animate-spin" aria-hidden="true" />}
            <span>{exporting ? 'מייצא...' : 'ייצא CSV'}</span>
          </button>
        </div>
      </div>
    </div>
