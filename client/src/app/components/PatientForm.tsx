...
export function PatientForm({ open, onClose, onCreated }: PatientFormProps) {
  ...
  return (
    ...
    >
      <div
        className="my-3 flex max-h-[calc(100vh-1.5rem)] w-full max-w-lg flex-col overflow-hidden rounded-xl border border-gray-200 bg-white shadow-xl sm:my-0 sm:max-h-[calc(100vh-3rem)]"
      ...
      >
        <div className="flex items-start justify-between gap-4 p-5 pb-4 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <h2 className="text-lg font-extrabold text-black">פתיחת תיק חדש</h2>
        ...
        <form onSubmit={handleSubmit} className="flex min-h-0 flex-1 flex-col">
          <div
            data-testid="case-form-scroll"
            className="min-h-0 flex-1 space-y-3.5 overflow-y-auto px-5 py-4"
          >
            <div>
              <label htmlFor="caseId" className="block text-sm font-bold text-gray-600 mb-1">מזהה תיק*</label>
              <input
                id="caseId"
                value={caseId}
              ...
              <p className="mt-1 text-xs text-gray-500">
                קוד פסאודונימי בלבד: אותיות באנגלית, מספרים, נקודה, מקף או קו תחתון.
              </p>
            </div>
            ...
            <fieldset>
              <legend className="block text-sm font-bold text-gray-600 mb-1">תאריך לידה*</legend>
              <div className="grid grid-cols-3 gap-2 sm:gap-3">
                <div>
                  <label htmlFor="birthDay" className="sr-only">יום לידה</label>
                  <input
                    id="birthDay"
                    value={birthDay}
                    ...
                    placeholder="יום"
                    inputMode="numeric"
                    className="h-10 w-full px-3 text-sm border border-gray-300 rounded-lg focus:border-black focus:ring-4 focus:ring-black/10 outline-none"
                  />
                </div>
                <div>
                  <label htmlFor="birthMonth" className="sr-only">חודש לידה</label>
                  <input
                    id="birthMonth"
                    value={birthMonth}
                ...
                <div>
                  <label htmlFor="birthYear" className="sr-only">שנת לידה</label>
                  <input
                    id="birthYear"
                    value={birthYear}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label htmlFor="dominantHand" className="block text-sm font-bold text-gray-600 mb-1">יד דומיננטית*</label>
                <select
                  id="dominantHand"
                  value={dominantHand}
                  onChange={(e) =>
              ...
              <div>
                <label htmlFor="educationYears" className="block text-sm font-bold text-gray-600 mb-1">שנות לימוד*</label>
                <input
                  id="educationYears"
          ...
          <div
            data-testid="case-form-footer"
            className="flex shrink-0 gap-3 border-t border-gray-100 bg-white p-5"
          >
            <button
              type="button"
              onClick={onClose}
