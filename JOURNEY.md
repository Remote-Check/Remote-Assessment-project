...
| Patient PWA delivery | Patient and clinician surfaces now build separately from the same repo. The patient surface has manifest/icons, iOS metadata, conservative app-shell service worker, route gating, staging banner, home-screen guidance, and tablet/phone task hardening. | Separate patient PWA deployment from the clinician website, with static app-shell caching only and no cached clinical evidence/API responses. | Hosted staging, licensed-stimuli validation, installed-PWA iPad/tablet QA, and phone fallback checks remain blocked on external execution before clinical pilot. |
